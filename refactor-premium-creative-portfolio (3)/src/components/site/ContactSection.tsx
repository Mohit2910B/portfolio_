...
export default function ContactSection({ data }: { data: SiteData }) {
  ...
  const validate = (): boolean => {
    ...
    if (digits.length < 5 || digits.length > 15) next.phoneNumber = "Enter a valid mobile number.";
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/enquiries", {
        ...
        body: JSON.stringify({
          ...form,
          countryCode: form.countryCode.split(" ")[0] || "+91",
        }),
      });
      ...
      if (!response.ok) {
        ...
        setMessage(payload.error ?? "Submission failed. Please try again.");
        setStatus("idle");
        return;
      }
      setStatus("sent");
      setMessage(payload.message ?? "Enquiry received.");
      setForm(EMPTY);
    } catch {
      setStatus("idle");
